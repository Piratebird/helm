...
content = content.replace('print the path to the configuration file', 'Print all system paths (Config, State, Downloads)')
...
content = re.sub(r'        if args\.logs:.*?sys\.exit\(0\).*?if args\.config_path:.*?sys\.exit\(0\)', '', content, flags=re.DOTALL)
...
content = content.replace('        args = parser.parse_args()', f'{argparse_additions}\n        args = parser.parse_args()')
